...
- **Google Sheets (ticket ##51, botón "Actualizar Sheet")**: integración separada del login, con su propio **service account** de Google Cloud (no reusa `GOOGLE_CLIENT_ID`/`SECRET`, que son para OAuth de usuario). Variables: `GOOGLE_SHEETS_SPREADSHEET_ID` (mismo Sheet real en dev y prod — no hay una copia separada para pruebas), `GOOGLE_SHEETS_CLIENT_EMAIL` + `GOOGLE_SHEETS_PRIVATE_KEY` (credenciales del service account, mismas en dev y prod), `GOOGLE_SHEETS_TAB_PREFIX` (`DEV_` en dev, vacío en prod — así los tabs de prueba no se mezclan con los reales). Paso manual único, fuera del código: compartir el Sheet real con el email del service account (acceso *Editor*) desde Google Drive.
